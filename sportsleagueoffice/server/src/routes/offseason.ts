...
async function setTeamStarters(): Promise<void> {
  ...
  for (const team of teams.rows) {
    ...
    for (const pos of positions) {
      const player = roster.rows.find((p: any) => p.position === pos && !usedIds.has(p.id));
      if (player) {
...
router.post('/new', authMiddleware(true), async (req: any, res) => {
  try {
    ...
    // Balance positions and set starters for all teams
    await balanceTeamPositions();
    await setTeamStarters();
