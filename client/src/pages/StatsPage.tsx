                ...
                {leaders?.map((player, idx) => (
                  ...
                  >
                    <span className={cn(
                      ...
                      idx === 0 ? 'text-amber-500' : idx === 1 ? 'text-slate-400' : idx === 2 ? 'text-orange-600' : 'text-slate-500'
                    )}>
                      {idx + 1}
                    </span>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-slate-500">{player.games_played} GP</p>
                    </div>
                  </Link>
                ))}
              </div>
                ...
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">ORtg</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">DRtg</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-slate-500">Net</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rankings.map((team, idx) => (
                      <tr key={team.team_id} className="border-t border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2">
                          ...
                          >
                            ...
                            <span className="text-sm font-medium truncate">{team.abbreviation}</span>
                          </Link>
                        </td>
                        <td className="px-3 py-2 text-sm text-right">{team.offensive_rating.toFixed(1)}</td>
                        <td className="px-3 py-2 text-sm text-right">{team.defensive_rating.toFixed(1)}</td>
                        <td className={cn(
                          'px-3 py-2 text-sm text-right font-medium',
                          team.net_rating > 0 ? 'text-green-600' : team.net_rating < 0 ? 'text-red-600' : 'text-slate-600'
                        )}>
